...
void RigidBodyComponent::getWorldTransform(btTransform &worldTrans) const
{
	...
	if (VERBOSE) std::cout << "RigidBodyComponent::getWorldTransform " << getPositionVec3FromTransform( this->owningGameObject->sceneNode.getTransformation(WORLD)) << std::endl;
}
...
{ 
	...
	for (auto bud : buddys) {
		...
		if (bud->getComponentType() == COLLISION) {

			static_cast<CollisionComponent*>(bud)->CollisionEnter(collisionData);

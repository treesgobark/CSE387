...
struct SubMesh {
	...
	GLuint vao; // ID for Vertex Array Object for the sub-mesh

	GLuint vertexBuffer; // ID for vertex data buffer for the sub-mesh

	GLuint indexBuffer; // ID for index buffer for the sub-mesh (if indexed rendering is used)

	Material* material; // Pointer to material properties including textures for the sub-mesh

	GLuint count; // Either the number of vertices in the mesh or the number of indices

	RENDER_MODE renderMode; // Render mode for the mesh. Either ORDERED or INDEXED
